import { scaleLinear, scaleBand } from "@seygai/visx-scale";
...
describe("getTickPosition(scale, align)", () => {
  describe("scales without band", () => {
    it("return center position", () => {
      const position = getTickPosition(
  ...
  describe("scales with band", () => {
    describe("align", () => {
      ...
      it("start", () => {
        expect(
          (getTickPosition(scale, "start")("b") as number).toFixed(2)
        ).toEqual("33.33");
      });
      it("end", () => {
        expect(
          (getTickPosition(scale, "end")("b") as number).toFixed(2)
        ).toEqual("66.67");
      });
    });
