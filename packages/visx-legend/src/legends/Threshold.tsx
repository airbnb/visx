...
import { PickD3Scale, ScaleInput } from "@seygai/visx-scale";
...
export type LegendThresholdProps<Scale extends AnyThresholdScale> = LegendProps<
  Scale
> &
  TransformProps & {
...
function defaultTransform<Scale extends AnyThresholdScale>({
  ...
  return ({ scale, labelFormat }) => {
    ...
    return (d, i) => {
      const [d0, d1] =
        scaleRange.length >= i
          ? scale.invertExtent(scaleRange[i])
          : [undefined, undefined];
      ...
      if (d0 == null && typeof d1 === "number") {
        ...
        // threshold step
        value = d;
        text = `${formatZero(labelFormat(d0, i))}${delimiter}${formatZero(
          labelFormat(d1, i)
        )}`;
