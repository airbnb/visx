...
import stackOffset from "@seygai/visx-shape/lib/util/stackOffset";
import stackOrder from "@seygai/visx-shape/lib/util/stackOrder";
import { StackPathConfig } from "@seygai/visx-shape";
...
>({ children, order, offset }: UseStackedData<Datum>) {
  ...
  const seriesChildren = useMemo(
    () =>
      React.Children.toArray(children).filter((child) =>
        isChildWithProps<ChildrenProps>(child)
      ),
    [children]
  ...
  const stackedData = useMemo(() => {
    ...
    const hasSomeNegativeValues = offset
      ? null
      : combinedData.some((d) => d.negativeSum < 0);
