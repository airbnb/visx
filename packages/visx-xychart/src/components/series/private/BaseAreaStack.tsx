...
import { LinePath, StackPathConfig } from "@seygai/visx-shape";
...
>({
  ...
  const accessors = useMemo(() => {
    ...
    return horizontal
      ? {
          y: (d: AreaStackDatum) =>
            (coerceNumber(yScale(getStackValue(d.data))) ?? 0) + yOffset,
          x0: (d: AreaStackDatum) =>
            (coerceNumber(xScale(getFirstItem(d))) ?? 0) + xOffset,
          x1: (d: AreaStackDatum) =>
            (coerceNumber(xScale(getSecondItem(d))) ?? 0) + xOffset,
      ...
      : {
          x: (d: AreaStackDatum) =>
            (coerceNumber(xScale(getStackValue(d.data))) ?? 0) + xOffset,
          y0: (d: AreaStackDatum) =>
            (coerceNumber(yScale(getFirstItem(d))) ?? 0) + yOffset,
          y1: (d: AreaStackDatum) =>
            (coerceNumber(yScale(getSecondItem(d))) ?? 0) + yOffset,
          defined: (d: AreaStackDatum) =>
    ...
    () =>
      stackedData.map((stack, stackIndex) => {
        const areaSeries:
          ...
          (child) => child.props.dataKey === stack.key
        );
        const {
          data,
          dataKey,
