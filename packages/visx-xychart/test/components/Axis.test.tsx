...
import VxAxis from "@seygai/visx-axis/lib/axis/Axis";
...
function setup(
  ...
  return mount(
    <DataContext.Provider
      value={{ ...getDataContext(series), ...contextOverrides }}
    >
      <svg>{children}</svg>
    </DataContext.Provider>
  );
...
describe("<AnimatedAxis />", () => {
  ...
  it("should render a VxAnimatedAxis", () => {
    expect(
      setup(<AnimatedAxis orientation="bottom" />).find(VxAnimatedAxis)
    ).toHaveLength(1);
  });
});
