...
export default function getChangelogAddition(tagName: string, prs: PR[]) {
  ...
  const contributors = Array.from(new Set(prs.map(pr => pr.user.login))).sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase()),
  );

  // categorize PRs. each PR may be in multiple categories tho it will have only one title
  const enhancements = prs.filter(pr => pr.labels.some(label => label.name.match(/enhancement/gi)));
